...
export async function POST(req: NextRequest) {
  ...
  const taxRate = getTaxRateForCustomer(customer) || 6.5;
  console.log(`📍 Applying tax rate for ${customer.address?.county || ''}, ${customer.address?.administrativeDistrictLevel1 || ''}: ${taxRate}%`);

  const locationRes = await locationsApi.listLocations();
  const location = locationRes.result.locations?.find(loc => loc.status === 'ACTIVE');
  ...
  try {
    ...
    const customerAddress = {
      ...
      administrativeDistrictLevel1: customer.address?.administrativeDistrictLevel1 || 'PA',
      postalCode: customer.address?.postalCode || '',
      county: customer.address?.county || '',
      country: 'US',
    };

    const sanitizedCounty = customer.address.county
      ?.toLowerCase()
      .replace(/[^a-z0-9._-]/g, '-') || 'default-county';
    ...
    const safeOrderResponse = {
      ...orderRequest,
      order: {
        ...orderRequest.order,
        lineItems: orderRequest.order.lineItems.map((item) => ({
          ...item,
          basePriceMoney: {
            ...item.basePriceMoney,
        ...
        taxes: orderRequest.order.taxes.map((tax) => ({
          ...tax,
    ...
    console.log('✅ Created order with taxes:', JSON.stringify(safeOrderResponse, null, 2));

    const checkoutResponse = await checkoutApi.createCheckout(SQUARE_LOCATION_ID, {
      idempotencyKey: new Date().toISOString() + '-checkout',
      order: {
        order: orderRequest.order,
      },
    ...
    const safeCheckoutResponse = convertBigIntToString({ ...checkoutResponse.result });
    console.log("🧾 CHECKOUT RESPONSE", JSON.stringify(safeCheckoutResponse, null, 2));

    const checkoutUrl = safeCheckoutResponse.checkout?.checkoutPageUrl;
    if (!checkoutUrl) {
      throw new Error("No checkout URL returned");
    }

    // ✅ Safely parse tax amount or fallback to taxRate
    const taxAmountStr = (safeOrderResponse.order as any)?.totalTaxMoney?.amount;
    const taxAmount = taxAmountStr ? Number(taxAmountStr) / 100 : taxRate;

    await sendOrderEmailToOwner(items, checkoutUrl, customer, taxAmount);

    return NextResponse.json({ checkoutUrl });

  } catch (error) {
    console.error('🛑 Square checkout error caught:', error);
    console.log('🔐 SQUARE_ACCESS_TOKEN Loaded:', !!SQUARE_ACCESS_TOKEN);
    console.log('📍 SQUARE_LOCATION_ID:', SQUARE_LOCATION_ID);

    if (error instanceof ApiError) {
      const errorMessage = error.message;
      const errorDetails = error.result?.errors ?? null;
      console.error('🚫 Square API Error Message:', errorMessage);
      console.error('📋 Square API Error Details:', JSON.stringify(errorDetails, null, 2));
    } else if (error instanceof Error) {
      console.error('⚠️ General Error Message:', error.message);
    } else {
